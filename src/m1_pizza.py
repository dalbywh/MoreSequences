...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(125, 50), rg.Point(75, 50)]
    circle = rg.Circle(rg.Point(100, 50), 25)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  10 burlywood dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(25, 25), 25)
    draw_points_on_circle(window, circle, 10, 'burlywood')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  100 slices, thin (thickness=3) black lines.'
    window = rg.RoseWindow(400, 400, title)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in generate_points_on_circle(circle, number_of_slices):
        new_line = rg.Line(circle.center, k)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    window.render(0.05)
    for k in range (len(generate_points_on_circle(circle,
                                                  number_of_segments))):
        new_line = rg.Line(generate_points_on_circle(circle,
                                                  number_of_segments)[k-1],
                           generate_points_on_circle(circle,
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  300 black lines on red circle, ' \
            'hops = 61.'
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(number_of_lines):
        new_line = rg.Line(points[k], points[(k + hops_to_next_point) % len(
            points)])
        new_line.thickness = thickness
        new_line.color = color
        new_line.arrow = 'last'
